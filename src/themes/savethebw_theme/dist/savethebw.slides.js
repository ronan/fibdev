...
/* harmony import */ var lodash_fp_map__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! lodash/fp/map */ "./node_modules/lodash/fp/map.js");
/* harmony import */ var lodash_fp_map__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(lodash_fp_map__WEBPACK_IMPORTED_MODULE_1__);
...
(function ($, Drupal) {
  ...
  Drupal.behaviors.savethebwSlides = {
    attach: function attach() {
      $('body').once('savethebw-slides').each(function () {
        var resizeMapSlides = function resizeMapSlides() {
          if (document.querySelector('.paragraph--type-map-slides')) {
            lodash_fp_map__WEBPACK_IMPORTED_MODULE_1___default()(function (container) {
              // Set the height on the map image container to the height of the
              // viewport
              var mapImages = container.querySelector('.field--name-savethebw-map-images');
              mapImages.style.height = "".concat(window.innerHeight, "px"); // Adjust the margin-top on the content container up by the same
              // amount

              var mapContent = container.querySelector('.field--name-field-slide-items');
              mapContent.style['margin-top'] = "-".concat(window.innerHeight, "px"); // Set the height on each slide to the full viewport

              lodash_fp_map__WEBPACK_IMPORTED_MODULE_1___default()(function (slide) {
                slide.style.height = "".concat(window.innerHeight, "px");
              }, document.querySelectorAll('.field--name-field-slide-items > div')); // Vertically center each image

              lodash_fp_map__WEBPACK_IMPORTED_MODULE_1___default()(function (slide) {
                var img = slide.querySelector('img');

                if (window.innerHeight > img.offsetHeight) {
                  var diff = window.innerHeight - img.offsetHeight;
                  slide.querySelector('div').style.top = "".concat(diff / 2, "px");
                }
        ...
        if (document.querySelector('.paragraph--type-map-slides')) {
          var slideObserver = function slideObserver(entries) {
            lodash_fp_map__WEBPACK_IMPORTED_MODULE_1___default()(function (entry) {
              // A little jQuery to grab the index to act on the corresponding
              // map slide image
              var position = $(entry.target).closest('.field__item').index();

              if (entry.isIntersecting) {
                if (position === 0) return; // Only add a scroll watcher when one doesn't exist already

                if (!entry.target.classList.contains('is-watched')) {
                  window.addEventListener('scroll', function () {
                    var _entry$target$getBoun = entry.target.getBoundingClientRect(),
                        top = _entry$target$getBoun.top; // From the bottom of the screen to the middle of the screen
                    ...
                    if (top < window.innerHeight && top > window.innerHeight / 2) {
                      var half = window.innerHeight / 2; // Percentage left to reach viewport midline

                      var remaining = (top - half) / half;
                      var opacity = 1 - remaining; // Going for a more chunky transition ala Patagonia, so
                      // round to the nearest 10%

                      opacity = Math.floor(opacity * 110 / 10) * 10 / 100;
                      $(entry.target).closest('.paragraph--type-map-slides').find(".paragraph--type-map-slide.paragraph--view-mode-images-only:nth-of-type(".concat(position + 1, ")")).css('opacity', opacity);
                    } else {
                      var $active = $(entry.target).closest('.paragraph--type-map-slides').find(".paragraph--type-map-slide.paragraph--view-mode-images-only:nth-of-type(".concat(position + 1, ")"));

                      var _opacity = $active.css('opacity');

                      $active.css('opacity', Math.round(_opacity));
                    }
                  });
          ...
          var observer = new IntersectionObserver(slideObserver, {});
          lodash_fp_map__WEBPACK_IMPORTED_MODULE_1___default()(function (container) {
            lodash_fp_map__WEBPACK_IMPORTED_MODULE_1___default()(function (slide) {
              observer.observe(slide);
              container.classList.add('is-loaded');
            }, container.querySelectorAll('.paragraph--type-map-slide.paragraph--view-mode-default'));
          }, document.querySelectorAll('.paragraph--type-map-slides')); // A hack to prevent the page loading at a scroll position within the map slides
...
/***/ 7:
/*!**************************************!*\
  !*** multi ./js/savethebw.slides.js ***!
  \**************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! ./js/savethebw.slides.js */"./js/savethebw.slides.js");


/***/ })

/******/ });
//# sourceMappingURL=savethebw.slides.js.map