...
/* harmony import */ var lodash_fp_map__WEBPACK_IMPORTED_MODULE_15__ = __webpack_require__(/*! lodash/fp/map */ "./node_modules/lodash/fp/map.js");
/* harmony import */ var lodash_fp_map__WEBPACK_IMPORTED_MODULE_15___default = /*#__PURE__*/__webpack_require__.n(lodash_fp_map__WEBPACK_IMPORTED_MODULE_15__);
/* harmony import */ var scroll_detector__WEBPACK_IMPORTED_MODULE_16__ = __webpack_require__(/*! scroll-detector */ "./node_modules/scroll-detector/dist/scroll-detector.module.js");
/* harmony import */ var lodash_trim__WEBPACK_IMPORTED_MODULE_17__ = __webpack_require__(/*! lodash/trim */ "./node_modules/lodash/trim.js");
...
(function ($, Drupal) {
  ...
  Drupal.behaviors.savethebwWave = {
    attach: function attach() {
      $('body').once('savethebw-wave').each(function () {
        ...
        var V2 = 34.8;
        var V3 = 46.5;
        var c1 = [0, 0, -199.6, 7.4, -475.8, 80.5];
        var c2 = [0, 0, -169.5, 28.2, -475.8, 45.7];
        var c3 = [0, 0, -455.8, 34.3, -709.4, 46.9];
        var C1 = [479.1, 148.7, 213, 100.8, 0, 49.1];
        var C2 = [457.6, 94.2, 213, 79.8, 0, 28.1];
        var C3 = [325.3, 106.8, 170.4, 80.8, 0, 0];
        var paths = []; // Generate paths for first morph from initial static to mid-animation

        lodash_fp_map__WEBPACK_IMPORTED_MODULE_15___default()(function (index) {
          var M = [M1[0] + (M2[0] - M1[0]) / 100 * (index + 1), M1[1] + (M2[1] - M1[1]) / 100 * (index + 1)];
          var V = V1 + (V2 - V1) / 100 * (index + 1);
          var C = [C1[0] + (C2[0] - C1[0]) / 100 * (index + 1), C1[1] + (C2[1] - C1[1]) / 100 * (index + 1), C1[2] + (C2[2] - C1[2]) / 100 * (index + 1), C1[3] + (C2[3] - C1[3]) / 100 * (index + 1), C1[4] + (C2[4] - C1[4]) / 100 * (index + 1), C1[5] + (C2[5] - C1[5]) / 100 * (index + 1)];
          var c = [c1[0] + (c2[0] - c1[0]) / 100 * (index + 1), c1[1] + (c2[1] - c1[1]) / 100 * (index + 1), c1[2] + (c2[2] - c1[2]) / 100 * (index + 1), c1[3] + (c2[3] - c1[3]) / 100 * (index + 1), c1[4] + (c2[4] - c1[4]) / 100 * (index + 1), c1[5] + (c2[5] - c1[5]) / 100 * (index + 1)];
          paths.push(lodash_trim__WEBPACK_IMPORTED_MODULE_17___default()("\n            M".concat(M.join(','), "\n            V126\n            h1260\n            V").concat(V, "\n            c").concat(c.join(','), "\n            C").concat(C.join(','), "\n            z\n          ").replace(/\n/g, '').replace(/[ ]+/g, ' ')));
        }, _toConsumableArray(Array(100).keys())); // Generate paths for second morph

        lodash_fp_map__WEBPACK_IMPORTED_MODULE_15___default()(function (index) {
          var M = [M2[0] + (M3[0] - M2[0]) / 100 * (index + 1), M2[1] + (M3[1] - M2[1]) / 100 * (index + 1)];
          var V = V2 + (V3 - V2) / 100 * (index + 1);
          var C = [C2[0] + (C3[0] - C2[0]) / 100 * (index + 1), C2[1] + (C3[1] - C2[1]) / 100 * (index + 1), C2[2] + (C3[2] - C2[2]) / 100 * (index + 1), C2[3] + (C3[3] - C2[3]) / 100 * (index + 1), C2[4] + (C3[4] - C2[4]) / 100 * (index + 1), C2[5] + (C3[5] - C2[5]) / 100 * (index + 1)];
          var c = [c2[0] + (c3[0] - c2[0]) / 100 * (index + 1), c2[1] + (c3[1] - c2[1]) / 100 * (index + 1), c2[2] + (c3[2] - c2[2]) / 100 * (index + 1), c2[3] + (c3[3] - c2[3]) / 100 * (index + 1), c2[4] + (c3[4] - c2[4]) / 100 * (index + 1), c2[5] + (c3[5] - c2[5]) / 100 * (index + 1)];
          paths.push(lodash_trim__WEBPACK_IMPORTED_MODULE_17___default()("\n            M".concat(M.join(','), "\n            V126\n            h1260\n            V").concat(V, "\n            c").concat(c.join(','), "\n            C").concat(C.join(','), "\n            z\n          ").replace(/\n/g, '').replace(/[ ]+/g, ' ').replace(/,-/g, '-')));
        }, _toConsumableArray(Array(100).keys()));
        paths = [].concat(_toConsumableArray(paths), _toConsumableArray(paths.reverse())); // If there's an inline wave, add a scroll listener

        if (document.querySelector('.inline-wave svg path')) {
          var path = document.querySelector('.inline-wave svg path');
          ...
          nextPath();
          scroll_detector__WEBPACK_IMPORTED_MODULE_16__["default"].on('scroll', function () {
            if (!window.doScrollBump) {
              window.doScrollBump = true;
              setTimeout(function () {
                return window.doScrollBump = false;
              }, 1000);
            }
          });
        }
...
/* harmony default export */ __webpack_exports__["default"] = (scrollDetector);
...
/***/ 3:
/*!************************************!*\
  !*** multi ./js/savethebw.wave.js ***!
  \************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! ./js/savethebw.wave.js */"./js/savethebw.wave.js");


/***/ })

/******/ });
//# sourceMappingURL=savethebw.wave.js.map