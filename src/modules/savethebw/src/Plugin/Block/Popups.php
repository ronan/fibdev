...
/**
 * Provides a site popups block
 *
 * @Block(
 *   id = "site_popups",
 *   admin_label = @Translation("Site popups"),
 * )
 */
class Popups extends BlockBase {

  /**
   * {@inheritdoc}
   */
  public function getCacheTags() {
    return Cache::mergeTags(parent::getCacheTags(), ['savethebw:popups']);
  }
  ...
  public function build() {
    ...
    if ($nid) {
      ...
      if ($current_node->bundle() == 'popup') {
        return [ $view_builder->view($current_node, 'overlay') ];
      }
    }
    ...
    foreach ($alerts as $nid) {
      ...
      if ($node->hasField('field_page_visibility') && !$node->field_page_visibility->isEmpty()) {
        ...
        if (($condition->evaluate() && !$condition->isNegated()) ||
          ...
          $content[] = $view_builder->view($node, 'overlay');
          break;
        }
      }
    }
